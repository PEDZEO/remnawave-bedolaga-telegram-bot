    ...
    async def get_referrer_top_referrals(
        ...
        for row in rows:
            referrals.append({
                ...
                "full_name": f"{row.first_name or ''} {row.last_name or ''}".strip() or (row.telegram_id and f"User {row.telegram_id}") or f"User #{row.id}",
        ...
        for data in top_referrers:
            ...
            if user:
                result.append({
                    ...
                    "full_name": f"{user.first_name or ''} {user.last_name or ''}".strip() or (user.telegram_id and f"User {user.telegram_id}") or user.email or f"User #{user.id}",
